$(document).ready(function () {
    ...
    $("body").on("click", "#addServiceBanner", function () {

        var serviceBannerTitle = $.trim($("#serviceBannerTitle").val());
        var serviceBannerSubTitle = $.trim($("#serviceBannerSubTitle").val());

        var image_name = $("#file").val();
        var files = $('#file')[0].files;

        if (serviceBannerTitle == "" || serviceBannerTitle == null) {
            messageDisplay("Please enter title", 1500, "error");
            $("#serviceBannerTitle").focus();
            return false;
        }
        if (serviceBannerSubTitle == "" || serviceBannerSubTitle == null) {
            messageDisplay("Please enter sub title", 1500, "error");
            $("#serviceBannerSubTitle").focus();
            return false;
        }
...
//        }
        ...
        Data.append('serviceBannerTitle', serviceBannerTitle);
        Data.append('serviceBannerSubTitle', serviceBannerSubTitle);
        ...
        $.ajax({
            url: BASE_URL + '/admin/add-service-banner',
            type: 'post',
            data: Data,
