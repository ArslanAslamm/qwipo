$(document).ready(function() {
    ...
    $("body").on("click", "#addCareer", function() {
        ...
        var careerTitle = $.trim($("#careerTitle").val());
        var careerContent = $.trim(tinymce.get('careerContent').getContent());
        var careerOrder = $.trim($("#careerOrder").val());
        if (careerTitle == "" || careerTitle == null) {
            messageDisplay("Please enter title", 1500, "error");
            $("#careerTitle").focus();
            return false;
        }
        if (careerContent == "" || careerContent == null) {
        ...
        if (careerOrder < 0 || careerOrder == null) {
            messageDisplay("Please enter order of Content", 1500, "error");
            return false;
        }
        var Data = new FormData();
        Data.append('careerTitle', careerTitle);
        Data.append('careerContent', careerContent);
        Data.append('careerOrder', careerOrder);

        $(".submit-btn").attr("disabled", "disabled");
        $.ajax({
            url: BASE_URL + '/admin/add-career',
            type: 'post',
            data: Data,
