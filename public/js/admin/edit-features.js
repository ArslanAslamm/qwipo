 ...
 $("body").on("click", "#editFeatures", function () {         
      
        var editFeatureTitle = $.trim($("#editFeatureTitle").val()); 
        var editFeatureContent =$.trim(tinymce.get('editFeatureContent').getContent());       
        var files = $('#file')[0].files;        
        var featuresId = $(this).attr("data-id");
        
        if (editFeatureTitle == "" || editFeatureTitle == null) {
            messageDisplay("Please enter title",1500,"error");           
            return false;
        }
        if (editFeatureContent == "" || editFeatureContent == null) {
            messageDisplay("Please enter content",1500,"error");           
            return false;
        }
         
            var Data = new FormData();         
            Data.append('editFeatureTitle', editFeatureTitle);
            Data.append('editFeatureContent', editFeatureContent);
            Data.append('featuresId', featuresId);
        ...
        $.ajax({
                url: BASE_URL + '/admin/update-features',
                type: 'post',
                data:Data,
