$(document).ready(function () {
    ...
    $("body").on("click", "#editStatistic", function () {
        ...
        var editStatisticsTitle = $.trim($("#editStatisticsTitle").val()); 
        var image_name = $("#file").val();
        var files = $('#file')[0].files;
        var statisticsId = $(this).attr("data-id");

        if (editStatisticsTitle == "" || editStatisticsTitle == null) {
            messageDisplay("Please enter title", 1500, "error");
            $("#editStatisticsTitle").focus();
            return false;
        }
        var Data = new FormData();
        Data.append('editStatisticsTitle', editStatisticsTitle);
        ...
        $.ajax({
            url: BASE_URL + '/admin/update-statistics',
            type: 'post',
            data: Data,
