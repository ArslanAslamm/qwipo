$(document).ready(function () {
   ...
   plugins: [
    ...
    $("body").on("click", "#addBlogs", function () {

        var blogTitle = $.trim($("#blogTitle").val());
        var blogUrl = $.trim($("#blogUrl").val());
        var datepicker = $.trim($("#datepicker").val());
        var description =$.trim(tinymce.get('blogDescription').getContent());
        ...
        if (image_name == "" || image_name == null) {
            messageDisplay("Please upload blog", 1500, "error");
            return false;
        }
    ...
    }).on("keyup","#blogTitle", function () {
        $("#blogUrl").val(stringSlug($(this).val()));
})
