 ...
 $("body").on("click", "#editServices", function () {         
        ...
        var editServiceTitle = $.trim($("#editServiceTitle").val()); 
        var editServiceContent =$.trim(tinymce.get('editServiceContent').getContent());       
        var files = $('#file')[0].files;        
        var serviceId = $(this).attr("data-id");
        
        if (editServiceTitle == "" || editServiceTitle == null) {
            messageDisplay("Please enter title",1500,"error");           
            return false;
        }
        if (editServiceContent == "" || editServiceContent == null) {
            messageDisplay("Please enter content",1500,"error");           
            return false;
        }
         
            var Data = new FormData();         
            Data.append('editServiceTitle', editServiceTitle);
            Data.append('editServiceContent', editServiceContent);
        ...
        $.ajax({
                url: BASE_URL + '/admin/update-services',
                type: 'post',
                data:Data,
